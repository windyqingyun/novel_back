...
//		System.out.println("dataScopeFilter: " + sqlString.toString());
	 ...
	 * 按照机构进行数据过滤
	 * @param entity 当前过滤的实体类
	 * @param alias xml中定义的表的别名，如果传入参数是null则默认设置为a
	 * @param officeColumnName 业务表中存储机构id的列名，如果传入参数是null则默认设置为office_id
	 * @param sqlMapKey sqlMap的键值，例如设置“dsf”时，调用方法：${sqlMap.sdf}
	 * @author zhangsc
	 */
	public static void dataScopeFilterByOffice(BaseEntity<?> entity, String sqlMapKey, String alias, String officeColumnName) {
		Office office = entity.getCurrentUser().getOffice();
		...
		if(StringUtils.isNotBlank(officeColumnName)){
			columnName = alias + officeColumnName;
		} else {
			columnName = alias + "office_id";
		}

		StringBuilder sqlString = new StringBuilder();
		
		sqlString.append(" AND " + columnName + " = '" + office.getId() + "'");
