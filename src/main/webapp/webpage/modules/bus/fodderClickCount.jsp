...
<body class="gray-bg">
	...
	<div class="col-sm-12">
		<div class="col-sm-12">
			...
			<div class="pull-right">
				<form:form id="searchForm" modelAttribute="fodderClick" action="${ctx}/bus/statistics/fodderClickCount" method="post" class="form-inline">
					<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
					<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
					<div class="form-group">
						<span>素材标题：</span>
							<form:input path="fodder.title" htmlEscape="false" maxlength="64" class="form-control input-sm"/>
						<c:if test="${fodderClick.groupType != '03'}"><span>用户名称：</span>
							<form:input path="providerUser.name" htmlEscape="false" maxlength="64" class="form-control input-sm"/></c:if>
						<span>分组类型：</span>
							<form:select path="groupType" class="form-control input-sm">
								...
								<form:option value="03" label="按照素材分组"></form:option>
							</form:select>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		<thead>
			<tr>
				<th class="">素材标题</th>
				<c:if test="${fodderClick.groupType != '03'}"><th class="">用户昵称</th></c:if>
				<th class="">点击数量</th>
				<th class="">素材所属机构</th>
				<c:if test="${fodderClick.groupType != '03'}"><th class="">用户所属机构</th></c:if>
				<c:if test="${fodderClick.groupType != '03'}"><th class="">创建时间</th></c:if>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="item">
			<tr>
				...
				<c:if test="${fodderClick.groupType != '03'}"><td>
