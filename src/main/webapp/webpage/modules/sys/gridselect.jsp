...
<body class="gray-bg">
	...
	<form:form id="searchForm" modelAttribute="obj" action="${url}" method="post" class="form-inline">
		...
		<div class="form-group">
			<c:set var="searchLables" value="${fn:split(searchLabel, '|') }"></c:set>
			<c:set var="searchKeys" value="${fn:split(searchKey, '|') }"></c:set>
			<c:forEach items="${searchKeys }" var="searchK" varStatus="status">
				<span>${searchLables[status.index] }:</span>
				<form:input path="${searchK}" htmlEscape="false" maxlength="50" class=" form-control input-sm"/>
				&nbsp;
			</c:forEach>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		<thead>
			<tr>
				<th width="40px"><input type="checkbox" class="i-checks"></th>
