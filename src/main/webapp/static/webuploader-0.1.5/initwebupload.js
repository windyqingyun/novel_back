function initWebUploadImg(inputValId) {
	...
	// 单个文件大小限制(单位：B)
	var FILE_SINGLE_SIZE_LIMIT = 3000000;
	// 选择文件按钮
	...
	var uploader = WebUploader.create({
		...
		// 文件接收服务端。
		server: 'sqsw/a/sys/uploadFile/upload',
		// 选择文件的按钮。可选。
...
function initWebUploadFiles(inputValId) {
	...
	var FILE_NUM_LIMIT = 1;
	// 单个文件大小限制(单位：B)
	var FILE_SINGLE_SIZE_LIMIT = 500000000;
	// 选择文件按钮
	var $pickBtn = $("#filePicker");
	// 选择文件-置灰按钮
	var $pickDisabledBtn = $("#filePicker_disabled");
	...
	var uploader = WebUploader.create({
		...
		accept: {
			title: 'files',
			extensions: 'doc,pdf,docx,ppt,mp4,pptx',
			mimeTypes: 'application/msword,application/pdf,.docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-powerpoint,video/mp4,.pptx,application/vnd.openxmlformats-officedocument.presentationml.presentation.psm,application/octet-stream'
			//extensions: 'docx',
			//mimeTypes: ''
		},
		method: 'POST',
