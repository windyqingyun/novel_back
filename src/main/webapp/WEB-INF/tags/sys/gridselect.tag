...
<%@ attribute name="clearBtn" type="java.lang.String" required="false" description="是否显示清空按钮"%>
	...
	<div class="input-group">
		...
		class="${cssClass}" style="${cssStyle}"/>
       		 <span class="input-group-btn">
	             ...
	             <c:if test="${not empty clearBtn and clearBtn eq '1'}">
	             	<button type="button" onclick="clearInputVal()" id="${id}ClearButton" class="btn <c:if test="${fn:contains(cssClass, 'input-sm')}"> btn-sm </c:if><c:if test="${fn:contains(cssClass, 'input-lg')}"> btn-lg </c:if>  btn-warning"><i class="fa fa-trash"></i>
	             </button></c:if>
       		 </span>
